...
// Create an Anthropic instance with the user's API key or the environment variable
...
export async function chatCompletion(req: Request, res: Response) {
  try {
    ...
    const completion = await anthropic.messages.create({
      model: model, // the newest Anthropic model is "claude-3-7-sonnet-20250219" which was released February 24, 2025
...
export async function analyzeResume(req: Request, res: Response) {
  try {
    ...
    // Claude doesn't have a JSON response format option, so we need to extract it
...
export async function generateCoverLetter(req: Request, res: Response) {
  try {
    ...
    res.json({
      coverLetter: completion.content[0].text,
    });
  } catch (error: any) {
    res.status(500).json({ 
